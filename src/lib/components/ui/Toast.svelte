...
<div class="fixed top-8 right-8 z-[9999] flex flex-col gap-2 pointer-events-none">
    {#each appContext.ui.toast.toasts as toast (toast.id)}
        ...
        <div
            ...
            aria-live="polite">
            <div
                class="flex items-center gap-3 px-4 py-3 rounded-md shadow-lg border border-border-main border-l-[3px] bg-bg-panel text-fg-default {colorClass.split(
                    ' ',
                )[1]}">
                ...
                <button
                    type="button"
                    class="flex items-center justify-center p-1 rounded transition-all bg-transparent border-none cursor-pointer text-fg-muted hover:bg-bg-hover hover:text-fg-default shrink-0"
