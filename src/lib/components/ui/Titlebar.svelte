<script lang="ts">
    ...
    let displayTitle = $derived(activeTab ? activeTab.customTitle || activeTab.title : "");
    let fullPath = $derived(activeTab?.path || "");
...
<div class="h-9 flex items-center select-none w-full border-b shrink-0 bg-bg-titlebar border-border-main" style="transform: translateZ(0);" data-tauri-drag-region>
    <!-- Left Section: Logo, Commands, Bookmarks, Preview -->
    <div class="flex items-center px-3 gap-1 pointer-events-auto shrink-0 z-10">
        <button class="hover:bg-white/10 rounded p-1 pointer-events-auto outline-none mr-2" onclick={() => toggleAbout()} use:tooltip={"About MarkdownRS"}>
        ...
        <button class="flex items-center justify-center hover:bg-white/10 rounded p-1.5 text-fg-muted transition-colors border-none outline-none" class:opacity-50={!isMarkdown} class:cursor-not-allowed={!isMarkdown} onclick={toggleSplit} use:tooltip={isMarkdown ? "Toggle Split Preview (Ctrl+\\)" : "Preview not available"}>
            {#if !isMarkdown}
    ...
    <!-- Center Section: Title/Path Display (Draggable) -->
    <div class="flex-1 flex items-center justify-center px-4 min-w-0 h-full overflow-hidden" data-tauri-drag-region>
        {#if displayTitle}
            <span class="text-xs text-fg-muted truncate opacity-60 hover:opacity-100 transition-opacity pointer-events-auto cursor-default" title={fullPath || "Unsaved"}>
                {displayTitle}
            </span>
        {/if}
    </div>
    ...
    <div class="flex h-full pointer-events-auto items-center shrink-0 z-10">
        <button class="h-full px-3 flex items-center justify-center hover:bg-white/10 text-fg-muted outline-none transition-colors" onclick={() => toggleSettings()} use:tooltip={"Settings (Ctrl+,)"}>
            <Settings size={14} />
        </button>

        <div class="w-px h-4 bg-border-light mx-1 opacity-50"></div>

        <button class="h-full w-12 flex items-center justify-center hover:bg-white/10 text-fg-muted outline-none transition-colors" onclick={() => appWindow.minimize()} use:tooltip={"Minimize"}><Minus size={16} /></button>
        <button class="h-full w-12 flex items-center justify-center hover:bg-white/10 text-fg-muted outline-none transition-colors" onclick={() => appWindow.toggleMaximize()} use:tooltip={"Maximize / Restore"}>
            {#if isMaximized}<Copy size={14} class="rotate-180" />{:else}<Square size={14} />{/if}
        </button>
        <button class="h-full w-12 flex items-center justify-center hover:bg-danger hover:text-white text-fg-muted outline-none transition-colors" onclick={closeApp} use:tooltip={"Close"}><X size={16} /></button>
    </div>
</div>
