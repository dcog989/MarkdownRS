    ...
    case "ArrowRight":
      ...
      if (focusedIndex >= 0 && focusedIndex < menuItems.length) {
        ...
        if (submenuContainer) {
          ...
          setTimeout(() => {
            const submenuItems = submenuContainer.querySelectorAll('[data-submenu="true"] button:not([disabled])');
            if (submenuItems.length > 0) {
              const firstSubmenu = asHTMLElement(submenuItems[0]);
              if (firstSubmenu) firstSubmenu.focus();
              updateMenuItems();
              const firstEl = submenuItems[0] ? asHTMLElement(submenuItems[0]) : null;
