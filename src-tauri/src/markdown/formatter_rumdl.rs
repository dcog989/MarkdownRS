...
pub fn format_markdown(content: &str, file_path: Option<&Path>, project_root: Option<&Path>) -> Result<String, String> {
    ...
    let result =
        fix_coordinator.apply_fixes_iterative(&rules, &[], &mut content_buf, &config, 100, file_path_buf.as_deref());
